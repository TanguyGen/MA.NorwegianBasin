...
```{r spatialschematic, fig.cap = "The spatial structure of StrathE2E; Ocean volumes and seafloor habitats. StrathE2E is built around a simplified spatial structure which represents shelf seas. These spatial units are connected to each other and to boundaries as shown to the right. The volumes connected to each spatial component are highlighted in blue.", out.width = "95%"}
knitr::include_graphics("img/schematic overhang.svg")
```
...
*The perimeter of the model domain is defined by a `r DDepth`m depth contour and the Norwegian coastline. Open ocean boundaries occur wherever there is no coastline.*

```{r habitats, fig.cap = "Map of the model domain. StrathE2E defines seabed sediment habitats as inshore (blues) or offshore (yellows). Within each zone, three sediment classes are represented – fine (muddy, 1), medium (sandy, 2) and coarse (gravel, 3). A fourth class (rock, 0) represents an absence of soft sediment. The overhang in the model domain which does not contact the sea floor is indicated in red. Sedimentary data are derived from the Norwegian Geological Survey and extended by the Random Forest model already deployed for the Barents Sea and North East Greenland (Laverick 2022).", out.width = "100%"}
...
In 2023 a Random Forest model was trained on a Norwegian Geological Survey sediment map to return a 1/100th degree resolution atlas of seabed sediment properties for the Barents Sea and North East Greenland [-@laverick2022].
...
Values for each sediment type derived from parametrised relationships between total organic nitrogen content of sediments (TON%, percent by weight), mud content (mud%, percent by weight) and median grain size (D~50~, mm).
...
We derived the area-proportions of seabed habitat in the inshore and offshore zones from output of the Random Forest model described by Laverick et al. [-@laverick2022].
...
data_frame(Habitat = c("S0", "S1", "S2", "S3", "D1", "D2", "D3", "D0"),
           Depth = c("Shallow","Shallow","Shallow","Shallow", "Deep","Deep","Deep","Deep"),
           Zone = c("Inshore","Inshore","Inshore","Inshore", "Offshore","Offshore","Offshore","Offshore"),
           Sediment = c("None (Rock)", "Fine", "Medium", "Coarse", "Fine", "Medium", "Coarse","None (Overhang)"),
           `Area Proportion` = physical$Value[proportions],
           `Grain size` = c(NA, physical$Value[grain[1:3]],physical$Value[grain[4:6]], NA),
           Porosity = c(NA, physical$Value[porosity[1:3]], physical$Value[porosity[4:6]], NA),
           Permeability = c(NA, physical$Value[permeability[1:3]], physical$Value[permeability[4:6]], NA),
           `Nitrogen content` = c(NA, physical$Value[nitrogen[1:3]],physical$Value[nitrogen[4:6]], NA)) %>% 
  group_by(Zone, Depth) %>% 
  gt() %>% 
...
Log-transformed porosity has been shown to have a sigmoidal relationship with log~10~(median grain size) (D~50~, mm) [@wilson2018]:

$$log_{10}(porosity) = p_3 + p_4\left(\frac{1}{1+e^{(\frac{-log_{10}(D_{50})-p_1}{p_2})}}\right)$$

We use this relationship to calculate porosity for sea bed sediments in the Norwegian Sea (`r table.ref("tab_habitat_proportions")`), using an alternative parameterisation to Wilson [@pace2021].
...
Whole sediment permeability can be related to the proportion of sediment classed as mud (D~50~ \< 62 μm) [@pace2021].
...
For the **Scavenger and carnivorous benthos** guild, the crab *Cancer pagurus* and the two sea urchins *Echinus esculentus* and *Strongylocentrotus droebachiensis* were elected to obtain timing dates [@bakke2019; @hamed2024; @james2012].
...
-   Atmospheric deposition of nitrate and ammonia from ISIMIP3a ([Inter-Sectoral Impact Model Intercomparison Project](https://data.isimip.org/10.48364/ISIMIP.759077.2); [@yang2022]) with SSP projects from ISIMIP3b used to force the NEMO-ERSEM coupled hydro-geochemical runs mentioned above.
...
-   Wave height, period, and direction from the ERA-5 reanalysis monthly means accessed through [CDS](https://cds.climate.copernicus.eu/cdsapp#!/dataset/10.24381/cds.f17050d7?tab=overview) for 1980-2021 [@hersbach2020].
...
These data are derived from satellite observations using the algorithm of Gohin [-@gohin2011].
...
Derived from HadGEM2-ES model output [@jones2011] which forces the NEMO-ERSEM model run used throughout our implementation.
...
Sourced from the "histsoc" files for a 1901 - 2021 hindcast as monthly averages [@yang2022], available from [CDS](https://cds.climate.copernicus.eu/cdsapp#!/dataset/10.24381/cds.f17050d7?tab=overview).
...
Freshwater inflow derived from HadGEM2-ES model output [@jones2011] which forces the NEMO-MEDUSA model run used throughout our implementation.
...
The flow weighted averaging of concentrations was calculated on the 5-day NEMO-ERSEM time step, before averaging to a monthly climatology of concentrations.
...
## Potentially time-varying parameters of the fishing fleet model {.unnumbered}
...
tourist_catch<-read_xlsx("Mikes_stories.xlsx", sheet = "Tourist1")%>%
  filter(Year>=2010&Year<2020)
tourist_catch%>%
  summarise(across(everything(), mean)) %>% # Compute column means
  mutate(Year = "Mean") %>%             # Add a new column for identification
  bind_rows(tourist_catch, .) %>%                       # Bind the means row to the original data frame
  gt()
```
\newpage
```{r speciestourism}
...
```
\newpage
### EU (including UK) fishing catch and activity in the Norwegian Sea {.unnumbered}
...
To approximate international catch we use the GFW data [@kroodsma2018] to calculate an inflation factor.
...
For the pinnipeds, we used discards data from gillnets fisheries from [@moan2021]. We used data of harbour and grey seals on the coast of Norway from the region 2 of Figure \@ref(fig:bycatchseals)). The data used estimates based on data from 2015 for harbour seals and for 2011 and 2017 for the grey seals (Table \@ref(tab:sealtable)). We used data from [@pedersen2021] to multiply the number of idividuals by their mean weight and then divided by the surface area of the whole domain  to obtain bycatch in $t \cdot m^{-2}$.
...
The bycatch of birds was estimated using the estimations from [@hildesofiefantoft2021] for the year 2018. The data was used to estimate the number of birds bycatch from gillnets fisheries. We used the table 6.1 of the report (Table \@ref(tab:birdtable)). We multiplied the number of birds bycatched by their mean weight from [@pedersen2021] to obtain a yearly mean weight of birds bycatch by statistical area. We kept only the statistical areas 00 and 05 . We then divided the weight by the surface area of each statistical area  to obtain estimations of bycatch in $t \cdot m^{-2}$.
...
\newpage
![(#tab:birdtable) From [@hildesofiefantoft2021], Table 6.1 (translated from Norwegian).](img/Discards_birds.png)
\newpage
...
To calculate the processing-at-sea rates, we used data from the Norwegian directorate for fisheries landings which reports both post-processed weight (produktvekt) and estimated round weight (Rundvekt). To obtain the part of the landings being rejected at sea as offal, we calculated $(Round weight-post-processed weight)/Round weight$. We thus obtainend the offal rates for each gear and guild for the Norwegian fleet. We considered the other fleets to have a similar ratio of offal to round weight as the Norwegian fleet.
...
A set of independent data, referred as "Target data", was collected in order to evaluate the model and to fit some of the parameters. The target data correspond to four csv tables:
- The first is a table of observed annual-based indices, summarising the annual mean value of a range of parameters including productions, consumption, landings, discards, bycatch, concentrations of chlorophyll and nitrogen, production/biomass ratios and other metabolic ratios. The dataset contains a mean value ad the standard deviation of the mean value (`r table.ref("tab_annualobs")`).
- The second isis a table of observed monthly-based indices, representing the monthly concentrations of surface chlorophylle with a median value and values for the lower and upper centiles (0.2 and 0.8).(`r table.ref("tab_monthlyobs")`).
- The third is a table of independently known regional harvest ratios for each guild for the whole model domain.

## Annual target data {.unnumbered}

The independent data of annual-indices of the ecosystem were taken from various sources and translated to $mMN \cdot m^{-2} \cdot y^{-1}$ using the conversion rates used in [@heath2021] (`r table.ref("tab_convert")`).

- The annual total primary production and chlorphyll concentrations was taken from COPERNICUS-GlobColour assuming redfield ratio [@europeanunion-copernicusmarineservice2022].
- The annual production of zooplankton was estimated from a simulation using the NORWECOM [@skogen2018] model ran in January 2025 by Morten Skogen.
- The annual productions of fish, birds, cetaceans and pinnipeds were estimated from the values of [@skaret2016].
- The annual productions of benthos were estimated from [@blanchard2002].
- The estimations of consumptions were taken from the data of [@skaret2016], [@pedersen2021], [@christensen2001] and [@blanchard2002].
- The landings and discards were calculated on the same way described in section Fishing fleet.
- The Production/Biomass ratios were calculated from [@blanchard2002], [@christensen2001], [@mackinson2008] and [@fredriksen1995].
- The other metabolic ratios for macrophytes were taken from [@broch2012] 
- Denitrification estimated from [@mctigue2016]
- Nitrate concentration in sediment from [@laverick2022]
- Nitrate concentration in water column from [@mork2024a]
- Bycatch of birds from [@baerum2019]
- Bycatch of seals and cetaceans from [@moan2016]

## Monthly target data {.unnumbered}

Monthly chlorophyll concentrations were taken from COPERNICUS-GlobColour assuming redfield ratio [@europeanunion-copernicusmarineservice2022].

\newpage
```{r convert_cap}
tab_annualobs_cap <- table_nums(name = "tab_convert", 
    caption = stringr::str_glue("Conversion from g to mMN."))
```

`r table_nums("tab_convert")`

```{r tab_convert}

Target_annual <- read.csv(str_glue("Files/nitrogen_carbon_WW_conversions_ed.csv"))%>%
  gt()
Target_annual
```
...
\newpage
```{r annualobs_cap}
tab_annualobs_cap <- table_nums(name = "tab_annualobs", 
    caption = stringr::str_glue("Annual-based indices of the state of the ecosystem in the study area for 2010-2019."))
```

`r table_nums("tab_annualobs")`

```{r tab_annualobs}

Target_annual <- read.csv(str_glue("Files/{implementation}/{Period}/Target/annual_observed_{toupper(implementation)}_{Period}.csv"),fileEncoding = "latin1")%>%
  filter(Use1_0==1)%>%
  select(-c(Use1_0,Name,Region, Time_period))

split_target <- Target_annual %>%
  mutate(group = ceiling(row_number() / 19)) %>%
  group_split(group, .keep = FALSE)

split_target[[1]] %>%
  gt()%>%
  fmt_number(
    columns = c("Annual_measure", "SD_of_measure"),
    decimals = 4,
    use_seps = FALSE
  ) %>%
  cols_align(
...
`r table_nums("tab_annualobs", display = "cite")` Continued.
...
```{r monthlyobs_cap}
tab_monthlyobs_cap <- table_nums(name = "tab_monthlyobs", 
    caption = stringr::str_glue("Monthly-based indices of the state of the ecosystem in the study area for 2010-2019."))
```
`r table_nums("tab_monthlyobs")`
```{r tab_monthlyobs}

Target_monthly <- read.csv(str_glue("Files/{implementation}/{Period}/Target/monthly_observed_{toupper(implementation)}_{Period}.csv"),fileEncoding = "UTF-8")
  
Target_monthly %>%
  select(-c(low_cent_value,upp_cent_value	))%>%
  drop_na()%>%
  gt() %>%
  fmt_number(
    columns = c("median", "lower_centile","upper_centile"),
    decimals = 4,
    use_seps = FALSE
  ) %>%
  cols_align(
