...
Physical_parameters[3,"Value"] <- filter(My_space, Shore == "Inshore", Depth == "S")$Elevation * -1  # Inshore_Shallow_layer_thickness_(m)
 ...
 Physical_parameters[5,"Value"] <- Physical_parameters[5,"Value"] - (Physical_parameters[5,"Value"] * 0.1) # Area_proportion_of_inshore_rock_habitat_s0_(sum_of_all_8_habitat_areas_must=1)
 Physical_parameters[6,"Value"] <- Physical_parameters[6,"Value"] - (Physical_parameters[6,"Value"] * 0.1)      # Area_proportion_of_inshore_sediment_habitat_s1_(muddy)_(sum_of_all_8_habitat_areas_must=1)
 Physical_parameters[7,"Value"] <- Physical_parameters[7,"Value"] - (Physical_parameters[7,"Value"] * 0.1)      # Area_proportion_of_inshore_sediment_habitat_s2_(sandy)_(sum_of_all_8_habitat_areas_must=1)
 Physical_parameters[8,"Value"] <- Physical_parameters[8,"Value"] - (Physical_parameters[8,"Value"] * 0.1)    # Area_proportion_of_inshore_sediment_habitat_s3_(gravelly)_(sum_of_all_8_habitat_areas_must=1)
 Physical_parameters[9,"Value"] <- Physical_parameters[9,"Value"] - (Physical_parameters[9,"Value"] * 0.1)     # Area_proportion_of_offshore_rock_habitat_d0_(sum_of_all_8_habitat_areas_must=1)
 Physical_parameters[10,"Value"] <- Physical_parameters[10,"Value"] - (Physical_parameters[10,"Value"] * 0.1)    # Area_proportion_of_offshore_sediment_habitat_d1_(muddy)_(sum_of_all_8_habitat_areas_must=1)
 Physical_parameters[11,"Value"] <- Physical_parameters[11,"Value"] - (Physical_parameters[11,"Value"] * 0.1)    # Area_proportion_of_offshore_sediment_habitat_d2_(sandy)_(sum_of_all_8_habitat_areas_must=1)
 Physical_parameters[12,"Value"] <- Physical_parameters[12,"Value"] - (Physical_parameters[12,"Value"] * 0.1)  # Area_proportion_of_offshore_sediment_habitat_d3_(gravelly)_(sum_of_all_8_habitat_areas_must=1)
...
#### Add new overhang parameters ####

Physical_parameters <- add_row(Physical_parameters, Value = 0.1, .before = 13,
        Description = "Area_proportion_of_deep_ocean_boundary_(false_seabed)_(sum_of_all_8_seabed_habitat_areas_plus_deep_ocean_boundary_must=1)") %>% 
  add_row(Value = 1, .before = 67,
        Description = "Proportion_of_offshore_depth_layer_occupied_by_phytoplankton_(set_a_value_0</=_1.0)") %>% 
  add_row(Value = 0.1205, .before = 70,
          Description = "Wellmixedness_of_inshore_zone_with_respect_to_inflow_from_offshore_(set_a_value_0</=_1.0)")
