...
Dredge_effort<- readRDS("./Objects/Mollusc dredge effort.rds")

                                         # Convert to same units as international effort

Non_Rest_total <- (readRDS("./Objects/IMR absolute fishing effort.rds")+ Dredge_effort  + # Import Norwegian fishing effort        
...
colnames(Rest) <- ifelse(
  colnames(Rest) == "Pelagic_Trawlers", "Pelagic_Trawlers_ALIEN",
  ifelse(colnames(Rest) == "Pelagic_Seiners", "Pelagic_Seiners_ALIEN", colnames(Rest))
)
empty_matrix<-matrix(rep(0,16), ncol = 2, byrow = TRUE)
Rest<-cbind(Rest,empty_matrix)
...
Non_Rest_habitat <- (readRDS("./Objects/IMR absolute habitat effort.rds") + Dredge + # Add Norwegian fishing effort        
  ...
  365 * (60 * 60) / domain_size    

International <- Non_Rest_habitat + Rest                            # Get international effort by gear and habitat

International[,"Recreational"] <- c(habitats$area[1:4],c(0,0,0,0))       # Distribute recreational activity across the inshore zone according to area
International[,"Harpoons"] <- c(habitats$area[1:4],c(0,0,0,0))             # Distribute whaling activity across the inshore zone according to area
...
International["Offshore Overhang", "Shelf_Trawlers_Seiners"] <- 0          # We consider that we have no shelf trawlers offshore 

International_proportion <-  t(t(International)/colSums(International))      # Scale as proportions within gears
International_proportion[is.na(International_proportion)] <- 0
