...
labelled <- st_intersection(st_make_valid(Edges), st_make_valid(grid)) %>% 
  ...
  characterise_flows(st_transform(domains, crs = st_crs(Edges)), precision = 10000) #%>% # In which direction? (in or out of box and with which neighbour)
...
## Remove offending transects, It's at the awkward join between the management areas.

labelled <- labelled[[2]] %>% 
  slice((labelled[[1]]$V3) * -1) %>% 
  characterise_flows(st_transform(domains, crs = st_crs(Edges)), precision = 10000) #%>% # In which direction? (in or out of box and with which neighbour)
